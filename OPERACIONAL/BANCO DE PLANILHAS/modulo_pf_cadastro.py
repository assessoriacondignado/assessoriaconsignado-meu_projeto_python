...
def formatar_cpf_visual(cpf_db):
    ...
    return str(cpf_db).strip() # Retorna cru, conforme solicitado

def limpar_normalizar_cpf(cpf_raw):
    if not cpf_raw: return ""
    return str(cpf_raw).strip() # Apenas remove espaços
...
def carregar_dados_completos(cpf):
    ...
    if conn:
        try:
            ...
            # Tenta buscar por cpf_ref ou cpf (Compatibilidade)
            col_fk = 'cpf_ref' 
...
def get_colunas_tabela(nome_tabela):
    # Simulação básica ou consulta real se necessário
    return []

# --- INTERFACE PRINCIPAL (REFORMULADA) ---
def interface_cadastro_pf():
    ...
    # Inicializa sessão de dados se não existir ou se for carregar edição
    ...
    with c_form:
        ...
        # Formulário direto (sem botões de inserir individual)
        valores_form = {}
        for campo in CONFIG_CADASTRO["Dados Pessoais"]:
            ...
            if campo['tipo'] == 'data':
                if val_inicial and isinstance(val_inicial, str):
                    try: val_inicial = datetime.strptime(val_inicial, '%Y-%m-%d').date()
                    except: val_inicial = None
                valores_form[campo['key']] = st.date_input(campo['label'], value=val_inicial, format="DD/MM/YYYY", key=key_widget)
            else:
                valores_form[campo['key']] = st.text_input(campo['label'], value=str(val_inicial) if val_inicial else "", key=key_widget)

    with c_extra:
        st.warning("📞 Contatos & Endereço (Opcional)")
        
        # Gestão simplificada de Listas
...
def salvar_pf_db(dados_completos, modo, cpf_original=None):
    ...
    try:
        ...
        if modo == "novo":
            ...
            # Se for edição, limpa satélites para recriar (estratégia simples)
            if cpf_original != cpf_limpo:
                # Se mudou o CPF, precisamos atualizar as referencias ou deletar e criar
                # Aqui vamos assumir update cascade ou delete manual
                pass
            
            # Remove dados antigos das tabelas satélites para reinserir atualizados
            col_fk = 'cpf_ref'
