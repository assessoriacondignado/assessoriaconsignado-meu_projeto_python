        ...
        if not df_lista.empty:
            ...
            for _, row in df_lista.iterrows():
                ...
                with c1:
                    ...
                    with b2:
                        if st.button("✏️", key=f"e_fast_{row['id']}"): 
...
def interface_pesquisa_ampla():
    c_voltar, c_limpar, c_spacer = st.columns([1, 1.5, 5.5])
    ...
    if conn:
        try:
            ...
            cur = conn.cursor()
            cur.execute("SELECT DISTINCT convenio FROM banco_pf.pf_emprego_renda WHERE convenio IS NOT NULL")
                ...
                for idx, campo in enumerate(campos):
                    with colunas_botoes[idx % 4]: 
                        if st.button(f"➕ {campo['label']}", key=f"add_{campo['coluna']}", use_container_width=True):
                            st.session_state['regras_pesquisa'].append({'label': campo['label'], 'coluna': campo['coluna'], 'tabela': campo['tabela'], 'tipo': campo['tipo'], 'operador': None, 'valor': ''})
                            st.rerun()
    ...
    with c_regras:
        ...
        for i, regra in enumerate(st.session_state['regras_pesquisa']):
            with st.container(border=True):
                ...
                if regra['tipo'] == 'data':
                    novo_valor = c_val.date_input("Data", value=None, key=f"val_{i}", label_visibility="collapsed")
                else:
    ...
    if st.session_state.get('executar_busca'):
        ...
        df_res, total = executar_pesquisa_ampla(regras_limpas, st.session_state.get('pagina_atual', 1))
        ...
        if not df_res.empty:
            ...
            with st.expander("📂 Exportar Dados", expanded=False):
                ...
                if not df_modelos.empty:
                    ...
                    idx_mod = c_sel.selectbox("Selecione o Modelo:", range(len(df_modelos)), format_func=lambda x: opcoes_mods[x])
                    modelo_sel = df_modelos.iloc[idx_mod]
                    
                    if c_btn.button("⬇️ Gerar Arquivos"):
                        with st.spinner("Analisando volume de dados..."):
                            df_total, _ = executar_pesquisa_ampla(regras_limpas, 1, 9999999)
                            lista_cpfs = df_total['cpf'].unique().tolist()
                            total_reg = len(lista_cpfs)
                            limite_lote = 200000
                            num_partes = (total_reg // limite_lote) + (1 if total_reg % limite_lote > 0 else 0)
                            
                            st.info(f"Gerando {num_partes} lote(s) para {total_reg} registros.")
                            for p in range(num_partes):
                                cpfs_lote = lista_cpfs[p*limite_lote : (p+1)*limite_lote]
                                df_final = pf_export.gerar_dataframe_por_modelo(modelo_sel['id'], cpfs_lote)
                                if not df_final.empty:
                                    csv = df_final.to_csv(sep=';', index=False, encoding='utf-8-sig')
                                    st.download_button(f"💾 Baixar Parte {p+1}", data=csv, file_name=f"export_p{p+1}.csv", key=f"dl_{p}")

            # --- ÁREA DE EXCLUSÃO EM LOTE ---
            with st.expander("🗑️ Zona de Perigo: Exclusão em Lote"):
                st.error(f"A exclusão será aplicada aos {total} clientes filtrados.")
                tipo_exc = st.selectbox("O que excluir?", ["Selecione...", "Cadastro Completo", "Telefones", "E-mails", "Endereços", "Emprego e Renda"])
                convenio_sel = None
                sub_opcao_sel = None
                if tipo_exc == "Emprego e Renda":
                    c_e1, c_e2 = st.columns(2)
                    convenio_sel = c_e1.selectbox("Qual Convênio?", lista_convenios)
                    sub_opcao_sel = c_e2.radio("Nível", ["Excluir Vínculo Completo (Matrícula + Contratos)", "Excluir Apenas Contratos"])
                
                if tipo_exc != "Selecione..." and st.button("Preparar Exclusão"):
                    st.session_state['confirm_delete_lote'] = True
                
                if st.session_state.get('confirm_delete_lote'):
                    st.warning(f"Excluir definitivamente {tipo_exc} de {total} clientes?")
                    if st.button("🚨 SIM, EXCLUIR"):
                        df_total, _ = executar_pesquisa_ampla(regras_limpas, 1, 999999)
                        ok, msg = executar_exclusao_lote(tipo_exc, df_total['cpf'].tolist(), convenio_sel, sub_opcao_sel)
                        if ok: 
                            st.success(msg)
                            time.sleep(2)
                            st.rerun()
