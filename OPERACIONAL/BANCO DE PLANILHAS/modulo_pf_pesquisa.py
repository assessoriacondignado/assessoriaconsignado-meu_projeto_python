...
def interface_pesquisa_rapida():
    c1, c2 = st.columns([2, 2]); busca = c2.text_input("🔎 Pesquisa Rápida (Nome/CPF)", key="pf_busca")
        ...
        if not df_lista.empty:
            ...
            for _, row in df_lista.iterrows():
                ...
                with c1:
                    ...
                    with b1:
                        if st.button("👁️", key=f"v_fast_{row['id']}", help="Visualizar"): pf_core.dialog_visualizar_cliente(str(row['cpf']))
                    with b2:
                        if st.button("✏️", key=f"e_fast_{row['id']}", help="Editar"): st.session_state.update({'pf_view': 'editar', 'pf_cpf_selecionado': str(row['cpf']), 'form_loaded': False}); st.rerun()
                    with b3:
                        if st.button("🗑️", key=f"d_fast_{row['id']}", help="Excluir"): pf_core.dialog_excluir_pf(str(row['cpf']), row['nome'])
...
def interface_pesquisa_ampla():
    ...
    if st.session_state.get('executar_busca'):
        ...
        if not df_res.empty:
            ...
            # --- SOLICITAÇÃO: EXPORTAR COM REGRA DE 200 MIL LINHAS E 1 MILHÃO ---
