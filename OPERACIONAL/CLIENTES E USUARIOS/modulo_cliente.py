...
def dialog_editar_carteira_config(dados):
    ...
    with st.form("form_edit_cart_conf"):
        ...
        n_origem = st.selectbox("Origem Custo (Tabela Fator)", options=[""] + lista_origens, index=idx_org)
...
def app_clientes():
    ...
    with tabs[3]: # Carteira
        ...
        with st.expander("📂 Nova Carteira (Produtos)", expanded=False):
            st.info("Cria carteiras e tabelas automaticamente."); df_pds = listar_produtos_para_selecao()
            if not df_pds.empty:
                with st.container(border=True):
                    cc1, cc2, cc3, cc4 = st.columns([3, 3, 2, 2]); idx_p = cc1.selectbox("Produto", range(len(df_pds)), format_func=lambda x: df_pds.iloc[x]['nome']); nome_cart = cc2.text_input("Nome Carteira"); status_cart = cc3.selectbox("Status", ["ATIVO", "INATIVO"])
                    if cc4.button("💾 Criar", key="b_c_c"):
                        if nome_cart: salvar_nova_carteira_sistema(int(df_pds.iloc[idx_p]['id']), df_pds.iloc[idx_p]['nome'], nome_cart, status_cart); st.rerun()
