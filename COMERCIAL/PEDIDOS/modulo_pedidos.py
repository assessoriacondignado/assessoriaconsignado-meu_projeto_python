...
# =============================================================================
# 4. PAINÉIS LATERAIS (SUBSTITUEM OS DIALOGS)
# =============================================================================
...
def painel_editar_pedido(ped):
    st.markdown(f"### ✏️ Editar Pedido: {ped['codigo']}")
    ...
    with st.form("form_painel_editar_ped"):
        c1, c2 = st.columns(2)
        
        # --- 1. CLIENTE ---
        try: curr_cli_idx = df_c[df_c['id'] == ped['id_cliente']].index[0]
        except: curr_cli_idx = 0
        idx_cli = c1.selectbox("Cliente", range(len(df_c)), index=int(curr_cli_idx), format_func=lambda x: f"{df_c.iloc[x]['nome']} ({df_c.iloc[x]['cpf']})")
        sel_cli = df_c.iloc[idx_cli]
        
        # --- 2. PRODUTO ---
        try: curr_prod_idx = df_p[df_p['id'] == ped['id_produto']].index[0]
        except: curr_prod_idx = 0
        idx_prod = c2.selectbox("Produto", range(len(df_p)), index=int(curr_prod_idx), format_func=lambda x: df_p.iloc[x]['nome'])
        ...
        if st.form_submit_button("💾 Salvar Alterações", type="primary"):
            ...
            if ok: 
                st.success(f"Salvo!"); 
                time.sleep(1); 
                st.session_state.ped_panel_active = False # Fecha painel
                st.rerun()
            else: 
                st.error(f"Erro: {msg}")
...
def painel_status_pedido(ped):
    ...
    with st.form("form_painel_status_ped"):
        ns = st.selectbox("Novo Status", lst, index=idx)
        mod = st.selectbox("Modelo de Mensagem", mods)
...
def painel_excluir_pedido(pid):
    ...
    st.warning("Tem certeza que deseja excluir este pedido? Esta ação não pode ser desfeita.")
    if st.button("Sim, Confirmar Exclusão", type="primary", use_container_width=True):
...
def painel_tarefa_pedido(ped):
    ...
    with st.form("form_painel_tarefa_ped"):
        dt = st.date_input("Previsão", datetime.now())
        obs = st.text_area("Observação da Tarefa")
...
def app_pedidos():
    ...
    # ABA 2: LISTA DE PEDIDOS (LAYOUT DE COLUNAS)
    with tab_lista:
        ...
        if 'ped_panel_width' not in st.session_state: st.session_state.ped_panel_width = 40 # Largura padrão
                    ...
                    for _, row in df.iterrows():
                        ...
                        with st.expander(f"{cor} [{row['status']}] {row['codigo']} - {row['nome_cliente']} {emp_show} | R$ {row['valor_total']:.2f}"):
                            ...
                            with gc2:
                                ...
                                st.write(f"**Prod:** {row['nome_produto']}")
                                st.write(f"**Qtd:** {row['quantidade']} | **Total:** :green[R$ {row['valor_total']:.2f}]")
                            ...
                            ts = int(time.time()) # Timestamp para unicidade da chave
                            
                            if c1.button("👤 Cliente", key=f"c_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('cliente', row); st.rerun()
                            if c2.button("✏️ Editar", key=f"e_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('editar', row); st.rerun()
                            if c3.button("🔄 Status", key=f"s_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('status', row); st.rerun()
                            if c4.button("📜 Histórico", key=f"h_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('historico', row); st.rerun()
                            if c5.button("🗑️ Excluir", key=f"d_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('excluir', row); st.rerun()
                            if c6.button("📝 Tarefa", key=f"t_{row['id']}_{ts}", use_container_width=True):
                                ativar_painel('tarefa', row); st.rerun()
        ...
        if col_painel and st.session_state.ped_panel_active:
            with col_painel:
                with st.container(border=True):
                    ...
                    st.session_state.ped_panel_width = cp2.slider(
                        "Largura da Janela (%)", 
                        min_value=20, max_value=90, 
                        value=st.session_state.ped_panel_width,
                        label_visibility="collapsed"
                    )
                    
                    st.divider()
                    
                    # Conteúdo do Painel (Roteador)
