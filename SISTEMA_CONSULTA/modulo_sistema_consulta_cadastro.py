            ...
            if tipo_dado == 'texto_vinculado':
                ...
                if "IS NULL" in operador_sql or "IS NOT NULL" in operador_sql:
                    if "NOT" in operador_sql:
                        where_clauses.append(f"({coluna} IS NOT NULL)")
                    else:
                        where_clauses.append(f"({coluna} IS NULL)")
                    ...
                    if filtro['val']:
                        ...
                        for v_str in valores:
                            ...
                            if v_str_limpo:
                                if coluna == 't.cpf':
                                    if 'LIKE' in operador_sql:
                                        ors.append(f"CAST({coluna} AS TEXT) {operador_sql} %s")
                                    else:
        ...
        try:
            with conn.cursor() as cur:
                if tipo == "DadosDinâmicos":
                    ...
                    """, (cpf_val, dados.get('cep'), dados.get('rua'), dados.get('bairro'), dados.get('cidade'), dados.get('uf')))
