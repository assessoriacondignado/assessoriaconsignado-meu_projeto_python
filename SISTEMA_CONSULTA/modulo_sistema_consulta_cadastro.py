...
def calcular_idade_completa(data_nasc):
    ...
    # Calculo aproximado de meses e dias para exibição
    ...
    if hoje.day >= data_nasc.day:
        ...
        # Simplificação: 30 dias se der erro no calculo exato do mês
        try:
            dias_no_mes = (date(ano_ref, mes_anterior + 1, 1) - date(ano_ref, mes_anterior, 1)).days
        except:
            dias_no_mes = 30
        dias = dias_no_mes + hoje.day - data_nasc.day
        meses -= 1 # Ajusta mês se dias "emprestados"
...
def tela_ficha_cliente(cpf, modo='visualizar'):
    ...
    # --- INJEÇÃO DE CSS (ESTILO SUBLINHADO E PRETO) ---
    st.markdown("""
        <style>
        /* Títulos dos Inputs (label) */
        .stTextInput label p, .stTextArea label p, .stDateInput label p, .stSelectbox label p {
            color: black !important;
            text-decoration: underline;
            font-weight: bold;
        }
        /* Texto dentro dos Inputs desabilitados */
        input:disabled {
            color: black !important;
    ...
    with st.expander("📄 Dados Cadastrais", expanded=False):
        ...
        idade_str = calcular_idade_completa(dt_nasc)
        if dt_nasc: dt_nasc = dt_nasc.strftime('%d/%m/%Y')
        
        c5.text_input("Data Nasc.", value=str(dt_nasc), disabled=True)
    ...
    with st.expander("📍 Contatos e Endereço", expanded=False):
        # Telefones e Emails em Grid
        if dados.get('telefones') or dados.get('emails'):
            cols_contato = st.columns(4)
            idx_c = 0
            
            for tel in dados.get('telefones', []):
                cols_contato[idx_c % 4].text_input(f"Telefone {idx_c+1}", value=tel['valor'], disabled=True)
                idx_c += 1
        ...
        if dados.get('enderecos'):
            for i, end in enumerate(dados.get('enderecos', [])):
                ...
                ce5.text_input("CEP", value=end.get('cep',''), key=f"cp_{i}", disabled=True)
        else:
    ...
    if financeiro:
        for (nome_convenio, matricula), grupo in financeiro.items():
            with st.expander(f"📋 {nome_convenio} - Matrícula: {matricula}", expanded=False):
                ...
                # Renderiza dados do convênio em grid compacta (4 colunas)
                if dados_esp:
