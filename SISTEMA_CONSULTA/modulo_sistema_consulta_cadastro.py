...
def buscar_cliente_rapida(termo):
    ...
    query = """
        ...
        WHERE nome ILIKE %s OR cpf ILIKE %s OR cpf = %s LIMIT 30
    """
    try:
        with conn.cursor() as cur:
            cur.execute(query, (f"%{termo}%", f"%{termo}%", termo_limpo if termo_limpo else '00000000000'))
            return cur.fetchall()
    finally:
        conn.close()
        ...
        if tipo_dado in ['texto_vinculado', 'endereco_vinculado']:
            ...
            if tipo_dado == 'texto_vinculado':
                ...
                if "IS NULL" in operador_sql:
                    ...
                    """
                    where_clauses.append(exists_clause)
                    params.append(val_final); params.append(val_final)
            ...
            if "IS NULL" in operador_sql:
                ...
                if operador_sql == 'BETWEEN':
                    ...
                    params.append(filtro['val'][0]); params.append(filtro['val'][1])
                else:
            ...
            if "IS NULL" in operador_sql or "IS NOT NULL" in operador_sql:
                if "NOT" in operador_sql: where_clauses.append(f"({coluna} IS NOT NULL AND {coluna} != '')")
                else: where_clauses.append(f"({coluna} IS NULL OR {coluna} = '')")
                ...
                if filtro['val']:
                    ...
                    for v in valores:
                        ...
                        if v:
                            ...
                            params.append(val_final)
                    if ors: where_clauses.append(f"({' OR '.join(ors)})")
...
def salvar_novo_cliente(dados_form):
    ...
    dados_limpos = {k: limpar_texto(v) for k, v in dados_form.items()}
    dados_limpos['data_nascimento'] = dados_form.get('data_nascimento')
    try:
        with conn.cursor() as cur:
            cols = list(dados_limpos.keys()); vals = list(dados_limpos.values())
            placeholders = ", ".join(["%s"] * len(cols)); columns = ", ".join(cols)
            cur.execute(f"INSERT INTO sistema_consulta.sistema_consulta_dados_cadastrais_cpf ({columns}) VALUES ({placeholders})", vals)
    ...
    try:
        with conn.cursor() as cur:
            if tipo == "Telefone":
                ...
                cur.execute("INSERT INTO sistema_consulta.sistema_consulta_dados_cadastrais_endereco (cpf, cep, rua, cidade, uf) VALUES (%s, %s, %s, %s, %s)", 
                            (cpf, limpar_texto(dados.get('cep')), limpar_texto(dados.get('rua')), limpar_texto(dados.get('cidade')), limpar_texto(dados.get('uf'))))
            elif tipo == "Convênio":
            ...
            conn.commit(); return "sucesso"
    except Exception as e: st.error(f"Erro inserir: {e}"); return "erro"
    finally: conn.close()
...
def atualizar_dados_cliente_lote(cpf, dados_editados):
    ...
    try:
        with conn.cursor() as cur:
            p = dados_editados['pessoal']
            cur.execute("""UPDATE sistema_consulta.sistema_consulta_dados_cadastrais_cpf SET nome=%s, data_nascimento=%s, identidade=%s, sexo=%s, cnh=%s, titulo_eleitoral=%s, nome_mae=%s WHERE cpf=%s""", 
                        (limpar_texto(p['nome']), p['data_nascimento'], limpar_texto(p['identidade']), limpar_texto(p['sexo']), limpar_texto(p['cnh']), limpar_texto(p['titulo_eleitoral']), limpar_texto(p['nome_mae']), cpf))
            ...
            for item in dados_editados.get('convenios', []):
                val = limpar_texto(item['valor'])
                if not val: cur.execute("DELETE FROM sistema_consulta.sistema_consulta_dados_cadastrais_convenio WHERE id=%s", (item['id'],))
                else: cur.execute("UPDATE sistema_consulta.sistema_consulta_dados_cadastrais_convenio SET convenio=%s WHERE id=%s", (val, item['id']))
            conn.commit(); return True
    except Exception as e: st.error(f"Erro atualizar: {e}"); return False
    finally: conn.close()
...
def excluir_cliente_total(cpf):
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit(); return True
    except Exception as e: st.error(f"Erro excluir: {e}"); return False
    finally: conn.close()
...
def modal_inserir_dados(cpf, nome_cliente):
    ...
    with st.form("form_insercao_modal"):
        dados_submit = {}
        if tipo_insercao == "Telefone": dados_submit['valor'] = st.text_input("Novo Telefone", placeholder="(00) 00000-0000")
        elif tipo_insercao == "E-mail": dados_submit['valor'] = st.text_input("Novo E-mail")
        elif tipo_insercao == "Endereço":
            dados_submit['cep'] = st.text_input("CEP"); dados_submit['rua'] = st.text_input("Rua")
            dados_submit['cidade'] = st.text_input("Cidade"); dados_submit['uf'] = st.text_input("UF", max_chars=2)
        elif tipo_insercao == "Convênio": dados_submit['valor'] = st.text_input("Nome do Convênio")
        
        if st.form_submit_button("✅ Salvar Inclusão"):
            status = inserir_dado_extra(tipo_insercao, cpf, dados_submit)
            if status == "sucesso": st.success("Sucesso!"); time.sleep(1); st.rerun()
            elif status == "duplicado": st.warning("Dado já existente!")
            else: st.error("Erro ao inserir.")

@st.dialog("⚠️ Confirmar Exclusão")
def modal_confirmar_exclusao(cpf):
    st.warning("Tem certeza que deseja excluir TODO o cadastro? Essa ação não pode ser desfeita.")
    if st.button("🚨 SIM, EXCLUIR DEFINITIVAMENTE", type="primary"):
        if excluir_cliente_total(cpf):
            st.success("Excluído!"); st.session_state['cliente_ativo_cpf'] = None
            st.session_state['modo_visualizacao'] = None; st.session_state['resultados_pesquisa'] = []
...
def tela_pesquisa():
    ...
    with tab1:
        ...
        if c2.button("Pesquisar", use_container_width=True):
            if len(termo) < 3: st.warning("Digite min. 3 caracteres.")
            else:
                res = buscar_cliente_rapida(termo)
                st.session_state['resultados_pesquisa'] = res
                if not res: st.warning("Nenhum cliente localizado.")

    # --- PESQUISA COMPLETA ---
    with tab2:
        # Layout: 40% Seleção | 60% Filtros
        col_selecao, col_filtros = st.columns([2, 3])
        
        # Lista temporária para sincronia imediata
        filtros_selecionados_agora = []
            ...
            for grupo, campos in MAPA_CAMPOS_PESQUISA.items():
                with st.expander(f"{grupo}", expanded=False):
                    ...
                    for i, (nome_campo, config) in enumerate(campos.items()):
                        # Coleta o estado diretamente do widget
                        chk_key = f"chk_{nome_campo}"
                        is_checked = cols_chk[i % 2].checkbox(nome_campo, key=chk_key)
                        if is_checked:
                            filtros_selecionados_agora.append((nome_campo, config))

        # --- DIREITA: INPUTS (Atualização Imediata) ---
        filtros_para_query = []
        with col_filtros:
            with st.warning("🌪️ Filtros Ativos (Configure abaixo)"):
                if not filtros_selecionados_agora:
                    st.info("Selecione colunas na esquerda para habilitar os filtros.")
                
                for i, (nome_campo, config) in enumerate(filtros_selecionados_agora):
                    ...
                    op_key = f"op_input_{nome_campo}"
                    val_key = f"val_input_{nome_campo}"
                    
                    c_op, c_val = st.columns([1.5, 2.5])
                    operador_escolhido = c_op.selectbox("Op", opcoes_ops, key=op_key, label_visibility="collapsed")
                    c_op.caption(f"ℹ️ {OPERADORES_SQL[tipo_ops][operador_escolhido]['desc']}")
                    ...
                    if valor_final is not None:
                        filtros_para_query.append({
                            ...
                            'val': valor_final, 'tipo': config['tipo'], 'table': config.get('table')
                        })
                    st.divider()
                ...
                if filtros_para_query or any("IS NULL" in f['op'] for f in filtros_para_query):
                    if st.button("🚀 EXECUTAR PESQUISA", type="primary", use_container_width=True):
                        res = buscar_cliente_dinamica(filtros_para_query)
                        st.session_state['resultados_pesquisa'] = res
                        if not res: st.toast("Nenhum registro encontrado.", icon="⚠️")

        # --- RESULTADOS (Container Abaixo) ---
        st.divider()
        with st.container():
            if 'resultados_pesquisa' in st.session_state and st.session_state['resultados_pesquisa']:
                ...
                for row in st.session_state['resultados_pesquisa']:
                    ...
                    c[0].write(str(row[0])); c[1].write(row[1]); c[2].write(row[2]); c[3].write(row[3])
    ...
    if st.button("➕ NOVO CADASTRO", type="primary"):
        ...
        st.session_state['modo_visualizacao'] = 'novo'; st.rerun()

def tela_ficha_cliente(cpf, modo='visualizar'):
    if 'modo_edicao' not in st.session_state: st.session_state['modo_edicao'] = False
    
    if st.button("⬅️ Voltar"):
        st.session_state['cliente_ativo_cpf'] = None; st.session_state['modo_visualizacao'] = None
    ...
    if modo == 'novo':
        ...
        with st.form("form_novo"):
            ...
            nome = c1.text_input("Nome*"); cpf_in = c2.text_input("CPF*")
            nasc = c3.date_input("Nascimento", value=None, min_value=date(1900,1,1), max_value=date(2050,1,1), format="DD/MM/YYYY")
            c4, c5, c6 = st.columns(3)
            rg = c4.text_input("RG"); sexo = c5.selectbox("Sexo", ["Masculino", "Feminino", "Outros"]); mae = c6.text_input("Nome da Mãe")
            if st.form_submit_button("💾 Salvar"):
                if salvar_novo_cliente({"nome": nome, "cpf": cpf_in, "data_nascimento": nasc, "identidade": rg, "sexo": sexo, "nome_mae": mae}):
                    st.success("Cadastrado!"); st.session_state['cliente_ativo_cpf'] = cpf_in
                    st.session_state['modo_visualizacao'] = 'visualizar'; time.sleep(1); st.rerun()
        return

    dados = carregar_dados_cliente_completo(cpf); pessoal = dados.get('pessoal', {})
        ...
        with st.container(key=chave_container):
            if st.session_state['modo_edicao']:
                with st.form("form_edicao_cliente"):
                    ...
                    edicoes_telefones = []; edicoes_emails = []; edicoes_convenios = []
                    with col_lista1:
                    ...
                    if fb1.form_submit_button("💾 CONFIRMAR ALTERAÇÕES", type="primary"):
                        pacote_dados = {
                            "pessoal": {"nome": e_nome, "identidade": e_rg, "data_nascimento": e_nasc, "cnh": e_cnh, "titulo_eleitoral": e_titulo, "sexo": e_sexo, "nome_mae": e_mae},
                            "telefones": edicoes_telefones, "emails": edicoes_emails, "convenios": edicoes_convenios
                        }
                        if atualizar_dados_cliente_lote(cpf, pacote_dados):
                            st.success("Dados atualizados!"); st.session_state['modo_edicao'] = False; time.sleep(1); st.rerun()
                ...
                with c_contato:
                    ...
                    for tel in dados.get('telefones', []): st.code(f"📱 {tel['valor']}")
                    for email in dados.get('emails', []): st.text(f"✉️ {email['valor']}")
                with c_endereco:
                    st.markdown("### 🏠 Endereços")
                    for end in dados.get('enderecos', []): st.info(f"{end.get('rua')}, {end.get('cidade')}/{end.get('uf')} - CEP: {end.get('cep')}")
                st.divider()
                st.markdown("### 💼 Convênios")
                st.write(", ".join([c['valor'] for c in dados.get('convenios', [])]))
                st.divider()
                col_ins_lat, _ = st.columns([1, 4])
                if col_ins_lat.button("➕ Inserir Dados Extras"): modal_inserir_dados(cpf, pessoal.get('nome'))
...
def app_cadastro():
    ...
    if st.session_state['modo_visualizacao'] == 'visualizar':
        ...
        if cpf_ativo: tela_ficha_cliente(cpf_ativo, modo='visualizar')
        else: st.session_state['modo_visualizacao'] = None; st.rerun()
